...
export class EntiteSimpleComponent<T extends EntiteSimple> extends EntiteComponent implements OnInit {
    ...
    public saveObject(): void {
        ...
        this.entiteSimpleService.httpPost(action, this.objectToSave)
            .subscribe(
                (result: EntiteResult<T>) => {
                    this.updatePageStatus(result.status, result.messages);
                    if (this.isSuccess()) {
